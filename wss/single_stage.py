...
def pseudo_gtmask(mask, ambiguous=True, cutoff_top=0.6, cutoff_bkg=0.6, cutoff_low=0.2, eps=1e-8, old_classes=16):
    ...
    bs, c, h, w = mask.size()
    mask = mask.view(bs, c, -1) # shape bs x (nb_classes + bkg) x 32*32

    # for each class extract the max confidence
    mask_max, _ = mask.max(-1, keepdim=True) # shape bs x c x 1
...
def balanced_mask_loss_ce(mask, pseudo_gt, gt_labels, ignore_index=255, old_classes=16, new_classes_mask=None):
    """Class-balanced CE loss
    - cancel loss if only one class in pseudo_gt
    - weight loss equally between classes

    Input args:
        mask: the logits coming out of the localizer
        pseudo_gt: the PAMR refined pseudo labels obtained from the localizer. these are hard labels
        gt_labels: one-hot labels for the current classes
    """

    # b,c,h,w = pseudo_gt.shape
    # mask shape: bs x tot_classes x h x w
    # pseudo_gt shape: bs x tot_classes x h x w
    mask = F.interpolate(mask, size=pseudo_gt.size()[-2:], mode="bilinear", align_corners=True)

    # indices of the max classes, or hard pseudo labels
    mask_gt = torch.argmax(pseudo_gt, 1) # bs x h x w
    ...
    loss = (new_classes_mask * loss).sum() / ((batch_weight * new_classes_mask).sum() + 1e-5)
