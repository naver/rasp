...
class BCEWithLogitsLossWithIgnoreIndex(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        n_cl = torch.tensor(inputs.shape[1]).to(inputs.device) # nb_classes + bkg
        ...
        if self.reduction == 'mean':
            ...
            # this is probably done to ignore the pixels which are marked with ignore_index.
            # since ignore_index is not a real label, the one-hot version of such samples
            # will have zeros everywhere in the target vector
