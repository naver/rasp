...
# $3 name of the exp. e.g. Rasp
# $4 number of WS incremental steps. e.g., 1 for task voc, 4 for task voc-5, etc

# some examples:
# bash run/run-coco-rasp.sh 0,1 voc rasp 1
# bash run/run-coco-rasp.sh 0,1 voc-5 rasp 4

# for running on slurm:
# sbatch run/run-coco-rasp.sh 0,1 voc-2 rasp 10
...
do
    ...
    exp --name ${exp_name} --step ${i} --weakly ${dataset_pars} --alpha 0.9 --lr ${lr} --step_ckpt $ckpt_path \
        --loss_de 1 --lr_policy warmup --affinity --semantic_similarity
done
